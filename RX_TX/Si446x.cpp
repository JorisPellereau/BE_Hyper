...
uint8_t si_fifo_info(uint8_t* fifoInfoBuf) {
  ...
  //SPI.transfer(0x03);     // RAZ Fifo RX et Fifo TX A VERIFIER !!!! MODIFICATION
	digitalWrite(SI_SS,1);
...
// == Reset des Fifos aux choix ==
void reset_fifo(uint8_t* fifoInfoBuf , uint8_t SI_RESET_FIFO_XX) {
  digitalWrite(SI_SS,0);
  SPI.transfer(SI_FIFO_INFO); // Get fifo info cmd
  SPI.transfer(SI_RESET_FIFO_XX);     // RAZ Fifo RX et Fifo TX A VERIFIER !!!! MODIFICATION
  digitalWrite(SI_SS,1);
  return si_get_cmd(2, fifoInfoBuf); // Read back info  
}
// ==============================
